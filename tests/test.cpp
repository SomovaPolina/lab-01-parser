#include <gtest/gtest.h>
#include <Student.hpp>
#include <fstream>
#include <Json.hpp>

TEST(Example, EmptyTest) {
  EXPECT_TRUE(true);
}

TEST(File_path, true_file_path) {
  std::string file_path = "../json_files/main.json";
  Table table;
  table.parsing_file(file_path);
  std::stringstream out;
  out << table << std::endl;
  std::cout << table.print_string_all() << std::endl;
  EXPECT_TRUE(true);

}


TEST(File_path, mistake_file_path) {
  std::string error1;
  std::string check =
      "Wrong path of json file: ../json_files/maaainnn.json";
  try {
    std::string file_path = "../json_files/maaainnn.json";
    Table table;
    table.parsing_file(file_path);
    std::stringstream out;
    out << table << std::endl;
  } catch (std::string ERROR){
    error1 = ERROR;
    std::cout << "mistake is here: " << ERROR << std::endl;
  }
  EXPECT_EQ(error1,check);
}

TEST(Value_input, test_debt) {
  std::string error1;
  std::string check =
      "Debt is wrong (not null, string or array)\n";
  try {
    std::string file_path = "../json_files/test_debt.json";
    Table table;
    table.parsing_file(file_path);
    std::stringstream out;
    out << table << std::endl;
  } catch (std::string ERROR){
    error1 = ERROR;
    std::cout << "mistake is here: " << ERROR << std::endl;
  }
  EXPECT_EQ(error1, check);
}


TEST(Value_input, test_group) {
  std::string error1;
  std::string check =
      "Group is wrong (not string or int number)\n";
  try {
    std::string file_path = "../json_files/test_group.json";
    Table table;
    table.parsing_file(file_path);
    std::stringstream out;
    out << table << std::endl;
  } catch (std::string ERROR){
    error1 = ERROR;
    std::cout << "mistake is here: " << ERROR << std::endl;
  }
  EXPECT_EQ(error1,check);
}

TEST(Value_input, test_avg) {
  std::string error1;
  std::string check =
      "Average is wrong (not null,string,float,int)\n";
  try {
    std::string file_path = "../json_files/test_avg.json";
    Table table;
    table.parsing_file(file_path);
    std::stringstream out;
    out << table << std::endl;
  } catch (std::string ERROR){
    error1 = ERROR;
    std::cout << "mistake is here: " << ERROR << std::endl;
  }
  EXPECT_EQ(error1,check);
}

TEST(Value_input, test_meta) {
  std::string error1;
  std::string check =
      "Items don't equal meta\n";
  try {
    std::string file_path = "../json_files/fail_count.json";
    Table table;
    table.parsing_file(file_path);
    std::stringstream out;
    out << table << std::endl;
  } catch (std::string ERROR){
    error1 = ERROR;
    std::cout << "mistake is here: " << ERROR << std::endl;
  }
  EXPECT_EQ(error1,check);
}

